...
export default function AdminBouquetsPage() {
  ...
  const handleSave = async () => {
      ...
      try {
        ...
        toast({ variant: 'destructive', title: 'Error', description: 'Failed to save bouquet.' });
      } finally {
        setIsSaving(false);
      }
  };
  ...
  const handleDelete = async (id: string) => {
      ...
      try {
          ...
          toast({ variant: "destructive", title: "Error", description: "Failed to delete bouquet." });
      }
  };
    ...
    <div>
        ...
        <CardContent className="p-0">
          <Table>
            ...
            <TableBody>
              {products.length === 0 && (
                  <TableRow>
                      <TableCell colSpan={6} className="text-center py-10 text-muted-foreground">No bouquets found.</TableCell>
                  </TableRow>
              )}
                ...
                <TableRow key={bouquet.id}>
                  <TableCell>
                      <div className="w-12 h-12 bg-muted rounded overflow-hidden">
                          {bouquet.images?.[0] ? 
                            <img src={bouquet.images[0]} alt={bouquet.name} className="w-full h-full object-cover" /> :
                  ...
                  <TableCell className="text-right">
                    <Button variant="ghost" size="icon" onClick={() => openEditDialog(bouquet)}>
                      <Edit className="h-4 w-4" />
                    </Button>
                    <Button variant="ghost" size="icon" className="text-destructive" onClick={() => handleDelete(bouquet.id)}>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
                <DialogTitle>{editingProduct ? 'Edit Bouquet' : 'Create Bouquet'}</DialogTitle>
                <DialogDescription>Add details for the bouquet product.</DialogDescription>
            </DialogHeader>
            <div className="grid gap-4 py-4">
                     ...
                     <div className="flex gap-4 items-start border p-4 rounded-md">
                         {formData.images?.[0] ? (
                             <div className="relative w-24 h-24 rounded overflow-hidden flex-shrink-0">
                                 ...
                                 <Button variant="ghost" size="icon" className="absolute top-0 right-0 h-6 w-6 bg-white/50" onClick={() => setFormData({...formData, images: []})}>
                                     <X className="h-3 w-3" />
                                 </Button>
                             </div>
                ...
                <div className="flex items-center space-x-4">
                     <div className="flex items-center space-x-2">
                        <Switch id="enabled" checked={formData.isEnabled} onCheckedChange={(c) => setFormData({...formData, isEnabled: c})} />
