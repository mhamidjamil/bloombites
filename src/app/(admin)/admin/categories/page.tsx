            ...
            <TableBody>
              ...
              {categories.map((cat) => (
                <TableRow key={cat.id}>
                  ...
                  <TableCell>
                      <span className={`inline-flex items-center px-2 py-1 rounded text-xs font-medium ${cat.type === 'product' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'}`}>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent>
            <DialogHeader>
                ...
                <DialogDescription>Add a new category for bouquets or custom items.</DialogDescription>
            </DialogHeader>
            <div className="grid gap-4 py-4">
                <div className="space-y-2">
                    <Label>Name</Label>
                    <Input value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} placeholder="e.g. Chocolates"/>
                </div>
                <div className="space-y-2">
                    <Label>Type</Label>
