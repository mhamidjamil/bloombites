  ...
  const handleGenerateDescription = async () => {
    if (selectedItems.length < 2) {
      toast({
        ...
        description: 'Please select at least 2 items to generate a description.',
      });
      return;
    }
          ...
          {categories.map((cat) => (
            <TabsContent key={cat.id} value={cat.id}>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 mt-4">
                {customItems.filter(item => item.category === cat.id).map(item => {
                    ...
                    >
                      <CardContent className="p-2 relative">
                        ...
                        <div className="aspect-square relative w-full mb-2">
                           {image && <Image src={image.imageUrl} alt={item.name} fill className="object-cover rounded-md" />}
                        </div>
                        <h3 className="font-semibold text-sm truncate">{item.name}</h3>
                        <p className="text-xs text-muted-foreground">PKR {item.price}</p>
                      </CardContent>
                    </Card>
                  )
                })}
      ...
      <div className="lg:col-span-1">
        <Card className="sticky top-24 shadow-lg">
          <CardContent className="p-6">
            <h2 className="text-2xl font-bold font-headline mb-4">Your Custom Bouquet</h2>
            <div className="min-h-[100px] max-h-[200px] overflow-y-auto space-y-2 pr-2">
              {selectedItems.length > 0 ? (
                selectedItems.map(item => (
                  <div key={item.id} className="flex justify-between items-center text-sm">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-muted-foreground">PKR {item.price}</span>
                      <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => handleItemToggle(item)}>
                ...
                <p className="text-sm text-muted-foreground text-center py-8">Select items to get started!</p>
              )}
            </div>
            
            <div className="border-t my-4 pt-4 space-y-4">
              <h3 className="text-lg font-semibold flex items-center gap-2"><Bot className="h-5 w-5" /> AI Assistant</h3>
              <p className="text-xs text-muted-foreground">Add a theme or occasion below, then generate a name and description for your unique bouquet!</p>
              ...
              {isGenerating ? (
                ...
                <div className="bg-background p-3 rounded-md border text-sm">
                  ...
                  {aiDescription && <p className="text-muted-foreground mt-1">{aiDescription}</p>}
                </div>
              )}
            ...
            <Button size="lg" className="w-full mt-4" disabled={selectedItems.length === 0}>
